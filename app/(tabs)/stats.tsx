...
const BADGES: Badge[] = [
  ...
  },
  {
    ...
    progress: (s) => Math.min(s.filter((x) => x.status === 'completed').length / 10, 1),
    progressLabel: (s) =>
  ...
  },
  {
    ...
    check: (s) => s.filter((x) => x.network === 'Eşarj' && x.status === 'completed').length >= 5,
    progress: (s) =>
      Math.min(s.filter((x) => x.network === 'Eşarj' && x.status === 'completed').length / 5, 1),
    progressLabel: (s) =>
  ...
  },
  {
    ...
    check: (s) =>
      s.filter((x) => x.status === 'completed').reduce((a, x) => a + x.energy_kwh, 0) >= 50,
    progress: (s) => {
      const total = s.filter((x) => x.status === 'completed').reduce((a, x) => a + x.energy_kwh, 0);
  ...
  },
  {
    ...
    check: (s) =>
      new Set(s.filter((x) => x.status === 'completed').map((x) => x.network)).size >= 3,
    progress: (s) =>
      Math.min(new Set(s.filter((x) => x.status === 'completed').map((x) => x.network)).size / 3, 1),
    progressLabel: (s) =>
...
export default function StatsScreen() {
  ...
  const stats = useMemo(() => {
    ...
    completed.forEach((s) => {
      if (!networkMap[s.network]) networkMap[s.network] = { count: 0, energy: 0 };
    ...
    completed.forEach((s) => {
      connectorMap[s.connector_type] = (connectorMap[s.connector_type] ?? 0) + 1;
    });
    const topConnector = Object.entries(connectorMap).sort((a, b) => b[1] - a[1])[0]?.[0] ?? '—';
    ...
    const avgDurationMin =
      durations.length > 0 ? durations.reduce((a, d) => a + d, 0) / durations.length : 0;
    ...
    const favoriteStation = Object.entries(stationMap).sort((a, b) => b[1] - a[1])[0]?.[0] ?? '—';
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.header}>
            ...
            <View>
              ...
              <Text style={styles.headerSubtitle}>Çevre ve cüzdanınıza katkınız</Text>
            </View>
          </View>
          ...
          <View style={styles.emptyStateInline}>
            ...
            <Text style={styles.emptyText}>
              İlk şarj oturumunuzu tamamladıktan sonra istatistikleriniz burada görünecek.
            </Text>
          </View>
          ...
          <View style={styles.badgeGrid}>
            {BADGES.map((badge) => (
              <View key={badge.id} style={[styles.badgeCard, styles.badgeCardLocked]}>
                ...
                <Text style={[styles.badgeTitle, styles.badgeTitleLocked]}>{badge.title}</Text>
      ...
      >
        ...
        <View style={styles.heroCard}>
          ...
          <Text style={styles.heroDesc}>
            ...
            <Text style={styles.heroHighlight}>{stats.co2SavedKg.toFixed(1)} kg</Text> daha az
            CO₂ salındı.
          </Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.financialMini}>
            <View style={styles.financialMiniItem}>
              ...
              <Text style={styles.financialMiniValue}>{stats.sessionCount}</Text>
            </View>
            <View style={styles.financialMiniItem}>
              <Text style={styles.financialMiniLabel}>Toplam Enerji</Text>
              <Text style={styles.financialMiniValue}>{stats.totalEnergy.toFixed(1)} kWh</Text>
            </View>
            <View style={styles.financialMiniItem}>
              <Text style={styles.financialMiniLabel}>Ortalama/Oturum</Text>
              <Text style={styles.financialMiniValue}>
                ₺{formatTl(stats.sessionCount > 0 ? stats.totalCost / stats.sessionCount : 0)}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          {stats.networkList.map((net) => (
            <View key={net.name} style={styles.networkRow}>
              ...
              <Text style={styles.networkPercent}>{net.percent.toFixed(0)}%</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.chartContainer}>
            {stats.monthlyData.map((month, idx) => {
              const barHeightPercent = stats.maxKwh > 0 ? month.kwh / stats.maxKwh : 0;
              const barHeight = Math.max(barHeightPercent * 100, month.kwh > 0 ? 4 : 2);
              return (
                  ...
                  <View style={styles.chartBarWrapper}>
                    <View
                      style={[
                        ...
                        {
                          height: barHeight,
                          backgroundColor: month.kwh > 0 ? '#00D26A' : '#1E293B',
        ...
        <View style={styles.card}>
          ...
          <View style={styles.profileGrid}>
            ...
            <View style={styles.profileItem}>
              ...
              <Text style={styles.profileItemValue}>{stats.avgKwh.toFixed(1)} kWh</Text>
            </View>
            <View style={styles.profileItem}>
              <View style={styles.profileItemIcon}>
            ...
            >
              <View
                style={[
                  ...
                  badge.earned ? styles.badgeIconWrapEarned : styles.badgeIconWrapLocked,
                ]}
              >
                {badge.earned ? (
