  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.sectionHint}>Şarj başlatmak için bir konnektör seçin</Text>
        ...
        {station.connectors.map((connector, idx) => (
          <TouchableOpacity
            ...
            style={[
              ...
              connector.status === 'out_of_service' && styles.connectorRowDisabled,
            ]}
            onPress={() => {
              if (connector.status !== 'out_of_service') {
                setSelectedConnector(connector === selectedConnector ? null : connector);
              }
            }}
            disabled={connector.status === 'out_of_service'}
          >
            <View style={styles.connectorLeft}>
              <ConnectorBadge type={connector.type} powerKw={connector.powerKw} status={connector.status} />
            </View>
            <View style={styles.connectorRight}>
              <Text style={styles.connectorPrice}>
                {connector.pricePerKwh != null ? `₺${connector.pricePerKwh}/kWh` : 'Fiyat bilgisi yok'}
              </Text>
...
const styles = StyleSheet.create({
  ...
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#0F1117' },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
